...
%Optional input v1, vn
%Output: coefficients b1,c1,d1,b2,c2,d2,...
n=length(x);
A=zeros(n,n);            % matrix A is mxm
r=zeros(n,1);            % where m=3n-3
...
%A(n,n)=2;r(n)=vn;
A(1,1:2)=[2*dx(1) dx(1)];r(1)=3*(dy(1)/dx(1)-v1); % clamped conditions
