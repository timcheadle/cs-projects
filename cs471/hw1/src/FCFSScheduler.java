...
public class FCFSScheduler extends Scheduler {
		...
		/**
		 * This process will enqueue a process on the CPU queue.  For FCFS, this method
		 * simply adds a process to the end of the list.
		 *
		 */
		...
		/**
		 * This method will dequeue a process from the CPU queue.  For FCFS, this method
		 * will take the first process waiting off the queue.
		 *
		 */
	...
	public void run() {		
		...
		// Check to see if CPU and IO device have completed a process.
            ...
            // If there is a process in the CPU, see if the process has completed
            // it's burst.  If so, set the CPU to not busy.
			if (usingCPU != -1) {
